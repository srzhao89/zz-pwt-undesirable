...
source("./Functions/coverage.mpi.illu.weighted.R")
...
for (i in 1:length(year1)) {
  ...
  #
  H1=apply(x1*wx1,1,sum)+apply(g1*wg1,1,sum)
  ##
  ...
  #
  H2=apply(x2*wx2,1,sum)+apply(g2*wg2,1,sum)
  ...
  YDIREC2=g2
  #
  tt=coverage.mpi.illu.weighted(x1=x1,y1=y1,x2=x2,y2=y2,
  ...
  res[i,4:6]=tt$estimate.OECD[1:3]
  res[i,7:9]=tt$estimate.NOECD[1:3]
  res[i,10]=tt$estimate.OECD[3]-tt$estimate.NOECD[3]
  res.that[i]=tt$pval
  #res[i,7]=tt$sig
  #res[i,8:9]=tt$bounds0[2,]
  #res[i,4:5]=tt$bounds1[2,]
  ...
  #
  if (tt$bounds1[3,1]>1 | 1>tt$bounds1[3,2]){
    res.star[i]=3
  } else if (tt$bounds1[2,1]>1 | 1>tt$bounds1[2,2]){
    res.star[i]=2
  } else if  (tt$bounds1[1,1]>1 | 1>tt$bounds1[1,2]){
    res.star[i]=1
  } else {
    res.star[i]=0
  }
  
  if (tt$bounds1.OECD[3,1]>1 | 1>tt$bounds1.OECD[3,2]){
    res.OECD.star[i]=3
  } else if (tt$bounds1.OECD[2,1]>1 | 1>tt$bounds1.OECD[2,2]){
    res.OECD.star[i]=2
  } else if  (tt$bounds1.OECD[1,1]>1 | 1>tt$bounds1.OECD[1,2]){
    res.OECD.star[i]=1
  } else {
    res.OECD.star[i]=0
  }
  
  if (tt$bounds1.NOECD[3,1]>1 | 1>tt$bounds1.NOECD[3,2]){
    res.NOECD.star[i]=3
  } else if (tt$bounds1.NOECD[2,1]>1 | 1>tt$bounds1.NOECD[2,2]){
    res.NOECD.star[i]=2
  } else if  (tt$bounds1.NOECD[1,1]>1 | 1>tt$bounds1.NOECD[1,2]){
    res.NOECD.star[i]=1
  } else {
    res.NOECD.star[i]=0
  }
...
tex = paste(tex,"\\\\")
write(tex,file="./Output/coverage-mpi-without-CO2-weighted.tex")
