...
rank.prod=rank(desc(estimate_individual),ties.method="min")
Countries=df1$Country_x
...
tab.rank=formatC(rank.prod,width=3)

tex=paste(tex,"&",tab,"&",tab.rank)
tex


tex1=paste(tex,"\\\\")
write(tex1,file="./Output/prod-2005-2019.tex")
...
#round(res.individual,3)

#estimate_individual=res.individual[ii1,]

res.individual[abs(res.individual)<1e-8] <- 0

#estimate_individual=res.individual[,c(1,6,11,15)]
estimate_individual=cbind(res.individual[,c(1,6,11,15)],apply(res.individual,1,mean))
round(estimate_individual,3)


tab=formatC(estimate_individual, width=6, digits=3,format="f")
#tex=paste(tex,"&",tab)
...
tex=paste(tex,"\\\\")
write(tex,file="./Output/eff-prod-2005-2019.tex")
